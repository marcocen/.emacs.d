...
** Org mode
   ...
   Use doct to more simply declare capture templates.

   I use a couple of capture templates, but not as much as I should. I have a
   basic todo, a journal (this is the one I use the least) and a dedicated doc
   for each place I work at. Nested inside each todo are the same three basic
   templates:
...
#+END_SRC
   ...
   The previously defined capture templates are accessed via org-capture and
   viewed with org-agenda.
...
#+END_SRC
   ...
   Configure yequake to have a drop down for org-capture anywhere.
